...
function addProcess() {
    process++;
    let rowHTML1 = `
                          <td class="process-id" >P${process}</td>
                          <td class="priority hide" ><input type="text" value="1"></td>
                          <td class="arrival-time" ><input type="text" value="0"> </td>
                          <td class="process-time cpu process-input"><input type="text" value="1"> </td>
                      `;
...
function setInput(input) {
    for (let i = 1; i <= process; i++) {
        ...
        let rowCells1 = document.querySelector(".main-table").rows[i].cells;
        ...
        let pta = [];
        for (let j = 0; j < ptn; j++) {
            pta.push(Number(rowCells1[3].firstElementChild.value));
        }
...
function outputAverageTimes(output) {
    ...
    avgrt /= process;
    let avgNTT = 0;
    output.normalisedTurnAroundTime.forEach((element) => {
        avgNTT += element;
    });
    avgNTT /= process;
    avgNTT = avgNTT.toFixed(2);
    // return [avgct, avgtat, avgwt, avgrt];
    return [avgNTT, avgtat, avgwt, avgrt];
}
...
function setOutput(input, output) {
    ...
    for (let i = 0; i < process; i++) {
        ...
        output.normalisedTurnAroundTime[i] = output.turnAroundTime[i] / input.totalBurstTime[i];
    }
...
function showGanttChart(output, outputDiv) {
    ...
    ganttChartHeading.classList.add("ganttchartHeading")
...
function addAvgTAT(outputDiv, output) {
    let miandiv = document.createElement("div");
    miandiv.classList.add("showavg")
    let avgTAT = document.createElement("span");
    avgTAT.innerHTML = "Average TurnaAound Time: ";
    miandiv.appendChild(avgTAT);
    let bold = document.createElement("span");
    let avgTATval = document.createElement("b");
    avgTATval.innerHTML = output.averageTimes[1];
    miandiv.appendChild(avgTATval);
    outputDiv.appendChild(miandiv);
}

function addAvgWT(outputDiv, output) {
    let miandiv = document.createElement("div");
    miandiv.classList.add("showavg")
    let avgWT = document.createElement("span");
    avgWT.innerHTML = "Average Waiting Time: ";
    miandiv.appendChild(avgWT);
    let bold = document.createElement("span");
    let avgWTval = document.createElement("b");
    avgWTval.innerHTML = output.averageTimes[2];
    miandiv.appendChild(avgWTval);
    outputDiv.appendChild(miandiv);
}

function addAvgNTT(outputDiv, output) {
    let miandiv = document.createElement("div");
    miandiv.classList.add("showavg")
    let avgWT = document.createElement("span");
    avgWT.innerHTML = "Average Normalised TurnAround Time: ";
    miandiv.appendChild(avgWT);
    let bold = document.createElement("span");
    let avgWTval = document.createElement("b");
    avgWTval.innerHTML = output.averageTimes[0];
    miandiv.appendChild(avgWTval);
    outputDiv.appendChild(miandiv);
}
...
function showOutput(input, output, outputDiv) {
    // console.log(output.averageTimes[1]);
    // console.log(output.averageTimes[2]);
    // outputDiv.insertAdjacentHTML("beforeend", "<hr>");
    // showTimelineChart(output, outputDiv);
    showFinalTable(input, output, outputDiv);
    // outputDiv.insertAdjacentHTML("beforeend", "<hr>");
    addAvgTAT(outputDiv, output);
    addAvgWT(outputDiv, output);
    addAvgNTT(outputDiv, output);
...
function myFunction() {
    alert("Oops! Please Enter atleast 1 data");
}
document.getElementById("calculate").onclick = () => {
    if(process==0)
        myFunction()
...
function addAnswer() {
    ...
    var element1 = document.getElementById("back");
    element1.classList.remove("hide");
}
